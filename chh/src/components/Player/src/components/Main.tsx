...
const Player: FC<PlayerProps> = ({
  ...
  const [playbackSpeed, setPlaybackSpeed] = useState(
    Number(localStorage.getItem("chikiimass-speed")) || 1
  );
  ...
  const [volume, setVolume] = useState(
    isNaN(parseInt(localStorage.getItem("chikiimass-volume") as string))
      ? 100
      : Number(localStorage.getItem("chikiimass-volume"))
  );
  const [isMuted, setIsMuted] = useState(
    Boolean(Number(localStorage.getItem("chikiimass-muted")))
  );
  ...
  useEffect(() => {
    ...
    localStorage.setItem("chikiimass-volume", String(volume));
    localStorage.setItem("chikiimass-muted", String(+isMuted));
  }, [volume, isMuted]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("chikiimass-speed", String(playbackSpeed));
  ...
  return (
    <>
      {poster && !pauseDidUpdate && (
        <img src={poster} className="chikiimass-poster" alt="chikiimass Poster" />
      )}
      <div
        ref={containerRef}
        className={`chikiimass-container ${
          hoverEnabled ? "chikiimass-controls-hovered" : ""
        }`}
      >
        {children ? (
        ...
        {((!loadedData && pauseDidUpdate) || (loading && !paused)) && (
          <div className="chikiimass-center">
            <CircularProgress />
          </div>
        )}

        {paused && pauseDidUpdate && (
          <div className="chikiimass-center" onClickCapture={handleScreenClicked}>
            <PauseEffect />
          </div>
        )}
        ...
        {!pauseDidUpdate && (
          <div className="chikiimass-center" onClickCapture={handleScreenClicked}>
            <Play className="chikiimass-icon-md" />
          </div>
        )}
        ...
        <div
          ...
          className={`chikiimass-controls ${
            paused || settingsActive ? "chikiimass-show" : ""
          }`}
        >
          <div
            ref={seekRef}
            ...
            className="chikiimass-seek"
          >
            <div className="chikiimass-seek-bar">
              <div
                style={{
                  width:
                    duration !== 0
                ...
                className="chikiimass-seek-left"
              ></div>
            </div>
            {seekPreview !== null && (
              <div
                className="chikiimass-seek-preview"
                style={{
                  left:
          ...
          <div className="chikiimass-controls-main">
            <div className="chikiimass-controls-left">
              <button
                ref={pauseButton}
                className="chikiimass-center-container chikiimass-tooltips-left"
                data-chikiimass-tooltips={
                  paused
              ...
              >
                {paused ? (
                  <Play className="chikiimass-icon-sm" />
                ) : (
                  <Pause className="chikiimass-icon-sm" />
                )}
              </button>

              <div className="chikiimass-volume-container">
                <button
                  ref={volumeButtonRef}
                  className="chikiimass-center-container"
                  data-chikiimass-tooltips={
                    isMuted || volume === 0
                ...
                >
                  {isMuted || volume === 0 ? (
                    <VolumeMuted className="chikiimass-icon-sm" />
                  ) : volume === 100 ? (
                    <VolumeFull className="chikiimass-icon-sm" />
                  ) : (
                    <VolumeHalf className="chikiimass-icon-sm" />
                  )}
                </button>
                <div className="chikiimass-volume-wrapper">
                  <input
                    className="chikiimass-volume-slider"
                    type="range"
                    min={0}
                    max={100}
                  ...
                  />
                  <div
                    className="chikiimass-volume-left-bar"
                    style={{ width: isMuted ? 0 : volume * 0.52 }}
                  ></div>
                </div>
              </div>

              <div className="chikiimass-time">
            ...
            <div className="chikiimass-controls-right">
              {Boolean(subtitles) && (
                <button
                  ref={subtitleButtonRef}
                  className={`chikiimass-center-container ${
                    subtitleIndex >= 0 ? "chikiimass-icon-underline" : ""
                  }`}
                  data-chikiimass-tooltips={
                ...
                >
                  <Subtitle className="chikiimass-icon-sm" />
                </button>
              )}
                  ...
                  <div ref={ref} style={{ position: "relative" }}>
                    <button
                      ...
                      {...(!settingsActive
                        ...
                        : {})}
                    >
                      <Cog className="chikiimass-icon-sm" />
                    </button>
              ...
              {pictureInPicture && (document as any)?.pictureInPictureEnabled && (
                <button
                  ...
                  data-chikiimass-tooltips="Picture in Picture"
                  onClickCapture={() => {
                    try {
                      let doc = document as any;
                ...
                >
                  <PictureInPicture className="chikiimass-icon-sm" />
                </button>
              )}

              <button
                className="chikiimass-center-container chikiimass-tooltips-right"
                ref={fullscreenToggleButton}
                data-chikiimass-tooltips={`${
                  onFullScreen
              ...
              >
                {onFullScreen ? (
                  <ExitFullScreen className="chikiimass-icon-sm" />
                ) : (
                  <FullScreen className="chikiimass-icon-sm" />
                )}
              </button>
            </div>
          </div>
